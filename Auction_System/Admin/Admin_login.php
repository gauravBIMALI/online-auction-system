...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($email_err) && empty($password_err)) {
    ...
    $sql = "SELECT id, name, email, password FROM user WHERE email = ? AND role = 'admin'";
        ...
        if (mysqli_stmt_num_rows($stmt) == 1) {
          ...
          if (mysqli_stmt_fetch($stmt)) {
            if (password_verify($password, $hashed_password)) {
              // Password is correct, so start a new session and save the email and name to the session
              session_start();
